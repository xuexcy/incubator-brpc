...
DEFINE_int32(stack_size_small, 32768, "size of small stacks"); // 2^15
DEFINE_int32(stack_size_normal, 1048576, "size of normal stacks"); // 2^20
DEFINE_int32(stack_size_large, 8388608, "size of large stacks"); //  2^23
DEFINE_int32(guard_page_size, 4096, "size of guard page, allocate stacks by malloc if it's 0(not recommended)"); // 2^12
DEFINE_int32(tc_stack_small, 32, "maximum small stacks cached by each thread"); // 2^5
DEFINE_int32(tc_stack_normal, 8, "maximum normal stacks cached by each thread"); // 2^3
...
int allocate_stack_storage(StackStorage* s, int stacksize_in, int guardsize_in) {
    const static int PAGESIZE = getpagesize(); // 获取页内存大小, 一般是4096(0x1000)，即4k
    const int PAGESIZE_M1 = PAGESIZE - 1; // (0xFFF)
    const int MIN_STACKSIZE = PAGESIZE * 2; // 最小_栈大小 (0x2000)
    const int MIN_GUARDSIZE = PAGESIZE; // 最小_栈保护区大小 (0x1000)

    // Align stacksize  对齐栈大小(得到比目标需求大的多个页大小)
    // [ + PAGESIZE_M1 ] 得到比目标需求大的大小
    // [ & (~PAGESIZE_M1) ] 将栈大小于页对齐
    const int stacksize =
        (std::max(stacksize_in, MIN_STACKSIZE) + PAGESIZE_M1) &
        ~PAGESIZE_M1; // (0xFFFFF000)
    ...
    if (guardsize_in <= 0) {
        ...
        s_stack_count.fetch_add(1, butil::memory_order_relaxed); // 申请的栈数量+1
        s->bottom = (char*)mem + stacksize; // 栈首地址mem + 栈大小 = 栈底大小
        s->stacksize = stacksize;
        s->guardsize = 0;
        if (RunningOnValgrind()) { // TODO(xcy)
        ...
        if (aligned_mem != mem) { // 如果mem是页对齐的，那这里一定是相等的
            LOG_ONCE(ERROR) << "addr=" << mem << " returned by mmap is not "
                "aligned by pagesize=" << PAGESIZE;
        }
        const int offset = (char*)aligned_mem - (char*)mem; // 如果页对齐了，offset一定是0
        if (guardsize <= offset ||
            mprotect(aligned_mem, guardsize - offset, PROT_NONE) != 0) { // 设置保护页不可访问时失败
            munmap(mem, memsize); // 回收分配的内存页
            PLOG_EVERY_SECOND(ERROR)
...
void deallocate_stack_storage(StackStorage* s) {
    ...
    if ((uintptr_t)s->bottom <= (uintptr_t)memsize) { // 栈底地址比memsize都小，说明哪里出问题了
        return;
    }
    s_stack_count.fetch_sub(1, butil::memory_order_relaxed); //分配的stack数减1
    if (s->guardsize <= 0) {
        free((char*)s->bottom - memsize); // 对应allocate_stack_storage中的malloc
    } else {
        munmap((char*)s->bottom - memsize, memsize); // 对应allocate_stack_storage中的mmap
    }
}

int* SmallStackClass::stack_size_flag = &FLAGS_stack_size_small; //设置stack size
